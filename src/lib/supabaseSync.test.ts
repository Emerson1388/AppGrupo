...
import { newEntityId, persistCheckinByToken, persistPost } from "./supabaseSync"
...
describe("persistCheckinByToken", () => {
  it("não finge sucesso local quando a nuvem não está ligada", async () => {
    const result = await persistCheckinByToken("token-invalido-xyz")
    expect(result.error).toBeTruthy()
    expect(result.treinoId).toBeUndefined()
  })
})
